...
static void userDriverSetParam (bool* bFinished, float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear){   
	...
	}else
	{
		if ((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 0.5 ) {    //用车速判断是否完成泊车
			*cmdSteer = constrain(-1,1,20*(targetangle -_caryaw)/3.14) ;
			...
			flag = 1; 
		}else if( ((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 100)&& ( flagt ==2 )) { //接近停车位时，控制车的朝向与车位一致，速度控制在2
			//*cmdSteer = 0;
			*cmdSteer = constrain(-1,1,20*(targetangle -_caryaw)/3.14) ;
		    ...
		    *cmdSteer = 1;
			...
			if (fabs(fabs(_caryaw)-fabs(targetangle))<0.015){
				flagt = 2;
			}
			flag = 3;
	...
	if(*bFinished)
	{
		if (((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 5 )&&!goahead) { //接近停车位时，控制车的朝向与车位一致，速度控制在2
			*cmdSteer =constrain(-1,1,-20*(targetangle -_caryaw)/3.14);

			if( _speed > 10 ){*cmdBrake = 0.1;*cmdGear = -1;*cmdAcc = 0;}
			else{*cmdBrake = 0;*cmdGear = -1;*cmdAcc = 0.3;}
			flag = 7;
		}
		else if (((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 500 && dist <7.2 )&&!goahead){//较接近停车位时，给一个大的转向，速度控制在10			
		    *cmdSteer = 1;
			if( _speed > 10 ){*cmdBrake = 0.2;*cmdGear = -1;*cmdAcc = 0;}
			else{*cmdBrake = 0;*cmdGear = -1;*cmdAcc = 0.1;}
			flag = 8;
	...
	if ((_carX - _lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 5000)
		printf("d:%f\n", (_carX - _lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY));
