...
static void userDriverSetParam (bool* bFinished, float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear){   
	...
	{
		if (_caryaw - _lotAngle<0.01 && _caryaw - _lotAngle>-0.01)
			dist2 = dist / 0.01;
		else
	...
	}else
	{
		...
		/*else if (sqrt((_carX - _lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY)) < 20 && flagt == 2 && _speed>-10&&accreverse)
		{
			...
			*cmdSteer = constrain(-1,1,-20*(_lotAngle -_caryaw)/3.14-sgn*dist*1.2) ;
			...
			flag = 2;
		} else if (((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 1500 && (dist2 <24 || flag==3 ))&&(flagt!=2)){//较接近停车位时，给一个大的转向	//1500 26
			#pragma region drift
			...
			*cmdGear = 1;
			...
			flag = 3;
		} else if ((_carX-_lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 6050 && dist < 56) { //提前将车控制在道路左侧
