	...
	if (sqrt((_carX - _lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY)) < 70)
	{
		...
		else
		{
			if ((_carX - _lotX) * (_carX - _lotX) + (_carY - _lotY) * (_carY - _lotY) < 2.5) {    //用车速判断是否完成泊车
				*cmdSteer = constrain(-1, 1, -20 * (targetangle - targetyaw) / 3.14);
...
//				reverse = true;
				if (targetangle > 1.5 * PI || targetangle < 0.5 * PI)
					*cmdSteer = constrain(-1, 1, -40 * (targetangle - targetyaw) / 3.14 - sgn * dist * 2.8);
				else
				{
					*cmdSteer = constrain(-1, 1, -40 * (targetangle - targetyaw) / 3.14 + sgn * dist * 2.8);
				}
			...
			{//较接近停车位时，给一个大的转向	
				...
				if (abs(dist) > 6)
				{
					...
					*cmdSteer = constrain(-0.15, -0.1, 0.1 * (targetangle - targetyaw));
				}

				if (abs(targetyaw - targetangle) < PI / 4.0)
					*cmdBrake = 1;
				if (abs(_speed) < 0.1) {
					flagt = 2;
				}
	...
	printf("flag:%d\t", flag);
	printf("targetangle:%.2f\ttargetyaw:%.2f\tangleerr:%.2f\t", targetangle, targetyaw,targetangle-targetyaw);
	printf("\n");
